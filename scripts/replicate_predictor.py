...
def main():
    ...
    for rep in range(REPS):
        ...
        aucs.append(s["auc"]); lifts.append(lift); best.append(h["best_epoch"])
    ...
    print(f"  shipped 40x400 (single draw): AUC 0.683, lift 3.67x")
    print(f"  every candidate run above shipped? {bool((a > 0.683).all())}")
    json.dump({"n_ep": N_EP, "wpe": WPE, "aucs": aucs, "lifts": lifts,
               "best_epochs": best, "shipped_auc": 0.683},
              open("reports/predictor_replication.json", "w"), indent=2)
